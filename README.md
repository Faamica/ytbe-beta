...
별도의 제거 프로그램이 없습니다. 아래 두 가지만 지우면 흔적이 남지 않습니다.

### 1. 앱 본체

- **Windows** — 압축을 풀어둔 폴더(`YouTubeBatchExtractor.exe`와 `_internal`이 함께 있는 폴더)를 통째로 삭제
- **macOS** — `YouTubeBatchExtractor.app`을 휴지통으로

### 2. 앱이 만든 설정 폴더

참가 코드, 첫 실행일, 업데이트 기록, 자동 내려받은 yt-dlp가 들어 있습니다.

- **Windows** — 탐색기 주소창에 아래를 붙여넣고 `YouTubeBatchExtractor` 폴더 삭제
  ```
  %LOCALAPPDATA%
  ```
- **macOS** — Finder에서 `⌘⇧G` 누르고 아래 경로로 이동해 폴더 삭제
  ```
  ~/Library/Application Support/YouTubeBatchExtractor
  ```

### 3. (선택) yt-dlp 캐시

유튜브 서명 처리에 쓰는 스크립트가 캐시되어 있습니다. 수백 KB 정도라 남겨두어도 무방합니다.

- **Windows** — `%APPDATA%\yt-dlp` 또는 `%LOCALAPPDATA%\yt-dlp`
- **macOS** — `~/.cache/yt-dlp`

> 다른 프로그램에서도 yt-dlp를 쓰신다면 함께 쓰는 폴더일 수 있으니 그때는 남겨두세요.

### 다운로드한 결과물은?

영상·목록(TSV)·자막·중복 방지 목록은 **여러분이 지정한 저장 폴더**에 그대로 있습니다.
앱을 지워도 사라지지 않으니, 필요 없으면 직접 정리하세요.

> ⚠️ **설정 폴더를 지우면 참가 코드와 사용 시작일 기록도 함께 사라집니다.**
> 다시 설치하면 코드를 한 번 더 입력해야 합니다. 코드는 잘 보관해 주세요.
